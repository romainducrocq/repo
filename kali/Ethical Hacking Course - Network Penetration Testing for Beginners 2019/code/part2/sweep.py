...
def parse_arguments():
    parser = argparse.ArgumentParser(usage = '%(prog)s [options] <subnet>', description = 'ip checker', epilog = 'python ipscanner.py 192.168.1 -f somefile.txt')
    ...
    if " " in args.filename:
        parser.error("There cannot be a whitespace in the filename")
